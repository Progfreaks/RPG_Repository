...
public class GuiMapHandler implements IGameElements, ICharacterDefs{
			...
			for (int i = 0; i < buttonMatrix.length; i++) {
				for (int j = 0; j < buttonMatrix[i].length; j++) {
					if (buttonMatrix[i][j] == clickedButton) {
						int[] coords = {j,i};
						facade.setClickedCoords(coords);
					}
				}
			}
			facade.sendPhase("FIGHT");
	...
	private ActionListener pickListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			facade.sendPhase("LEVER");


		}
	};
	
	private ActionListener cureListener = new ActionListener() {
...
//	public void setCycle(GuiGameCycle c){
//		cycle = c;
//	}
	...
	public void setFacede(Facade facade){
		this.facade = facade;
	}
		...
		for (int i = 0; i < buttonMatrix.length; i++) {
			for (int s = 0; s < buttonMatrix[i].length; s++) {
				...
				case FLOOR_ELEMENT:// Freies Feld. MoveEvent
					...
					break; 
				case LEVER_ELEMENT:// Item. MoveEvent + PickUpEvent
					buttonMatrix[i][s].addActionListener(pickListener);
					break; 
				case CURE_ELEMENT:
					buttonMatrix[i][s].addActionListener(cureListener);
					break; 
				case ZOMBIE_ELEMENT :
					single.addEnemy(single.createEnemy(ZOMBIE));
					buttonMatrix[i][s].addActionListener(fightListener);
					break;
				case GHOST_ELEMENT:
					
					single.addEnemy(single.createEnemy(GHOST));				
					buttonMatrix[i][s].addActionListener(fightListener);
					break;
				case POT_ELEMENT:
					single.addEnemy(single.createEnemy(POT));					
					buttonMatrix[i][s].addActionListener(fightListener);
					break;
				case SLEIM_ELEMENT:
					single.addEnemy(single.createEnemy(SLEIM));					
					buttonMatrix[i][s].addActionListener(fightListener);
					break;
				case MUMMY_ELEMENT:
					single.addEnemy(single.createEnemy(MUMMY));					
					buttonMatrix[i][s].addActionListener(fightListener);
					break;
				}
...
//		int yPlayerCoord = game.getYCharCoord();
		int xPlayerCoord = single.getXCharCoord();
		int yPlayerCoord = single.getYCharCoord();
...
//		int yPlayerCoord = game.getYCharCoord();
		...
		int yPlayerCoord = single.getYCharCoord();
		boardMatrix = game.getBoardMatrix();
		//boardMatrix = facade.getBoardMatrix();
	...
	public void removeCureActionCall(int x,int y){
		if(buttonMatrix != null) buttonMatrix[y][x].removeActionListener(cureListener);
	}
...
//		int xPlayerCoord = facade.getXCharCoord();
//		int yPlayerCoord = facade.getYCharCoord();
		int xPlayerCoord = single.getXCharCoord();
		int yPlayerCoord = single.getYCharCoord();
		System.out.println("player before x "+xPlayerCoord+"\n");
		System.out.println("player before y "+yPlayerCoord+"\n");
		...
		int[] coords = {xClicked,yClicked};
//		facade.setClickedCoords(coords);
		facade.setCoords(coords);
