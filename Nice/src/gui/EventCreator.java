...
import gui.events.CharSelectEvent;
...
public class EventCreator implements ActionListener {
	...
	private JButton buttons[]; //<- Buttonarray des HeroSelect

	// Aufzaehlung fuer Event-Type
	public enum EVENT_TYPE {
		DEFAULT, NEW_GAME, ROLL, MOVE, PICKUP, FIGHT, CHARACTER, CONFIRM;
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(eventType == EVENT_TYPE.CHARACTER){
			buttons = guiMgr.getCharArray();
			int pos = 0;
			...
			CharSelectEvent charEvent = new CharSelectEvent(pos);
			thread = new Thread(charEvent);
			thread.start();
	...
	public void setCharCalls(JButton b){
		b.addActionListener(new EventCreator(EVENT_TYPE.CHARACTER));
	}
	/**
	 * Setter fuer das Button Array
	 * @param array
	 */
	public void setButtons(JButton[] array){
		buttons = array;
	}
